    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', background: 'white', padding: '20px', borderRadius: '8px' }}>
        ...
        >
          ...
          <option value="read">Deja lus</option>
        </select>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        {filteredMessages.length === 0 ? (
          <p style={{ textAlign: 'center', color: '#7f8c8d' }}>Aucun message trouve.</p>
        ) : (
            ...
            <div key={msg._id} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                ...
                <span style={{ fontSize: '0.85rem', color: '#7f8c8d' }}>
                  {new Date(msg.createdAt).toLocaleDateString()} a {new Date(msg.createdAt).toLocaleTimeString()}
                </span>
              </div>
              ...
              <div style={{ marginTop: '15px', display: 'flex', gap: '10px' }}>
                ...
                <button
                  onClick={() => { if (window.confirm('Supprimer ce message ?')) deleteMessage(msg._id); }}
